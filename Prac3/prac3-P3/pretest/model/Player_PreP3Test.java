...
public class Player_PreP3Test {
	...
	public void testUseItemInHand() {
		try {
			...
			player.selectItem(0);
			assertNull(player.useItemInHand(6));
			
			player.addItemsToInventory(block);
			player.selectItem(0);
			assertNotNull(player.useItemInHand(1));
			
			player.addItemsToInventory(tool);
			player.selectItem(0);
			assertNotNull(player.useItemInHand(3));
			...
			for (int dy=-1; dy<2; dy++)
				for (int dz=-1; dz<2; dz++) {
					try {
						...
						assertEquals(orient, player.getOrientation());
	...
	/*Test orientate en una orientación hacía el propio player */
	...
	public void testOrientateException2() throws BadLocationException, EntityIsDeadException  {
		Location loc = player.orientate(0, 0, 0);
	}
	
	/*Test orientate en una orientación hacía una posición no adyacente */
	@Test(expected = BadLocationException.class)
	public void testOrientateException4() throws BadLocationException, EntityIsDeadException  {
		Location loc = player.orientate(5, 0, 0);
	}
