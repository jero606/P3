...
public class Player extends LivingEntity {
	...
	public Location getOrientation() {
		return new Location(orientation.getWorld(), orientation.getX() + location.getX(), orientation.getY() + location.getY(), + orientation.getZ() + location.getZ());
	}
	 ...
	 * @return objeto que tengo en la mano
	...
	public Location move(int dx, int dy, int dz) throws EntityIsDeadException, BadLocationException {
		...
		if (isDead()) {
			throw new EntityIsDeadException();
		} else if (!posicionDestinoEnAdyacentes || !Location.check(destino) || (!destino.isFree() && !getLocation().getWorld().getBlockAt(destino).getType().isLiquid())) {
