...
public class BlockWorld {
	...
	/**
	 * Variable de CollectedItemsScore
	 */
	private CollectedItemsScore itemsScore;

	/**
	 * Variable de Miningscore
	 */
	private MiningScore miningScore;

	/**
	 * Variable de PlayerMovementScore
	 */
	...
	public void movePlayer(Player player, int dx, int dy, int dz) throws EntityIsDeadException, BadLocationException {
		...
		if (world.getBlockAt(loc) != null && world.getBlockAt(loc).getType().isLiquid()) {
			...
			movementScore.score(player.getLocation());
		...
		if (world.getItemsAt(loc) != null) {
			player.addItemsToInventory(world.getItemsAt(loc));
		...
		if (danyo != 0) {
			try {
				...
				if (world.getBlockAt(player.getOrientation()) != null
						...
						&& !world.getBlockAt(player.getOrientation()).getType().isLiquid()) {
					if (((SolidBlock) world.getBlockAt(player.getOrientation())).breaks(danyo)) {
						player.useItemInHand(times);
						miningScore.score(world.getBlockAt(player.getOrientation()));
	...
	private void play(Scanner sc) {
		...
		if (constructorWorld == 0) {
			...
			String nameWorld = sc.nextLine();
			constructorWorld = 1;
			createWorld(seed, size, nameWorld, namePlayer);
		}
	...
	public String showPlayerInfo(Player player) {
		...
		try {
			...
			resultado += "Scores: [items: " + itemsScore.getScoring() + ", blocks: " + miningScore.getScoring()
					+ ", movements: " + movementScore.getScoring() + "]\n";
			return resultado;
	...
	/**
	 * Getter del itemsScore
	 * 
	 * @return the itemsScore
	 */
	...
	/**
	 * Getter del mininScore
	 * 
	 * @return the miningScore
	 */
	...
	/**
	 * Getter del movementScore
	 * 
	 * @return the movementScore
	 */
